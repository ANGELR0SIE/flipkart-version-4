...
export function attachFilterListeners() {
  ...
  const clearAllButton = document.querySelector(
    ".remove-items.sec2-remove-items"
  );
  if (clearAllButton) {
...
export function updateSelectedFilters() {
  ...
  const allSelectedFilters = [...brandFilters, ...ramFilters, ...ratingFilters];

  if (!(priceRange.min === 0 && priceRange.max === Infinity)) {
    const displayMaxPrice =
      priceRange.max === Infinity ? "30000+" : priceRange.max;
...
function updateClearButtonsVisibility() {
  document.querySelectorAll(".filter-section").forEach((section) => {
    ...
    if (clearButton) {
      // Check if clearButton is not null
      clearButton.style.display = checkboxes.length > 0 ? "block" : "none";
    }
  });
}
...
function clearPriceFilter() {
  ...
  const maxPriceSelect = (document.getElementById("max-price").value =
    "30000+");
